 ...
 *  识别语言
 */
@property (nonatomic) TBDVoiceRecognitionLanguage language;

/**
 *  识别模式
 */
@property (nonatomic) TBDVoiceRecognitionProperty recognitionProperty DEPRECATED_ATTRIBUTE;

/**
 *  识别模式
 */
@property (nonatomic, copy) NSArray *recogPropList;
 ...
 *  资源类型
 */
@property (nonatomic) TBDVoiceRecognitionResourceType resourceType;
 ...
 *  语音请求UA（用于定制网络请求UA）
 */
@property (nonatomic, copy) NSString *speechRequestUA;

/**
 *  轻应用UID
 */
@property (nonatomic, copy) NSString *lightAppUID;

/**
 *  是否禁止标点
 */
@property (nonatomic) BOOL disablePuncs;

/**
 *  请求通讯录识别，需要通过BDSDataUploader上传通讯录数据，否则该项设置无效
 */
 ...
 *  是否禁止操作AudioSession
 */
@property (nonatomic) BOOL disableAudioSessionControl;

/**
 *  显示效果
 */
@property (nonatomic) TBDRecognizerResultShowMode resultShowMode;

/**
 *  提示音开关
 */
@property (nonatomic) TBDRecognizerPlayTones recordPlayTones;

/**
 *  是否在对话框启动后展示引导提示，而不启动识别，默认关闭，若开启，请确认设置提示语列表
 */
@property (nonatomic) BOOL isShowTipsOnStart;

/**
 *  引擎启动后3秒没检测到语音，是否在动效下方随机出现一条提示语。如果配置了提示语列表，则默认开启
 */
@property (nonatomic) BOOL isShowTipAfter3sSilence;

/**
 *  未检测到语音异常时，将“取消”按钮替换成帮助按钮。在配置了提示语列表后，默认开启
 */
@property (nonatomic) BOOL isShowHelpButtonWhenSilence;
 ...
 *  是否需要对录音数据进行压缩
 */
@property (nonatomic) BOOL isNeedCompress;

/**
 *  识别策略,TBDVoiceRecognitionStrategy
 */
@property (nonatomic) int recogStrategy;

/**
 *  在线识别的等待时间
 */
@property (nonatomic) NSTimeInterval onlineWaitTime;
